...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ error: 'Λάθος διεύθυνση email ή κωδικός πρόσβασης' });
    }
    ...
    res.status(200).json({
      ...
      token: await jwt.sign({ id: user._id }, config.TOKEN_SECRET, {
        expiresIn: '1d',
      }),
    });
  } catch (err) {
  ...
  try {
    if (req.params.userId === req.userId) {
      return res.status(400).json({
        error:
          'Δεν χρειάζεται να γίνει αίτημα σύνδεσης προς τον ίδιο το χρήστη',
      });
    }
  ...
  try {
    ...
    if (!request.receiver.equals(req.userId)) {
      return res.status(403).json({
        error: 'Λειτουργία διαθέσιμη μόνο για τον αποδέκτη του αιτήματος',
      });
    }
...
exports.deleteContactRequest = async (req, res, next) => {
  try {
    ...
    if (request) {
      ...
      if (
        !request.sender.equals(req.userId) &&
        !request.receiver.equals(req.userId)
      ) {
        return res.status(403).json({
          error: 'Λειτουργία διαθέσιμη μόνο για τα διαπλεκόμενα μέρη',
        });
      }
...
exports.getContacts = async (req, res, next) => {
  try {
    ...
    if (req.params.userId !== req.userId) {
      ...
      if (!isContact)
        return res.status(403).json({
          error: 'Δεν έχετε την άδεια για προβολή δικτύου του χρήστη',
        });
    }
...
exports.changeEntryStatus = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ message: 'Επιτυχής αλλαγή ιδιωτικότητας δεδομένων προφίλ' });
  } catch (err) {
    next(err);
  }
};
...
exports.deleteEntry = async (req, res, next) => {
  try {
    await User.updateOne(
      ...
      {
        $pull: {
          [`${req.params.entryType}.entries`]: { _id: req.params.entryId },
        },
      }
    );
...
exports.export = async (req, res, next) => {
  try {
    ...
    if (type === 'json') {
      ...
      const xml = new j2xParser().parse(
        JSON.parse(JSON.stringify({ user: users }))
      );
